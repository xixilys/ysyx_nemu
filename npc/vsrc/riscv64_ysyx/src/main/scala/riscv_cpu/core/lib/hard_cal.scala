...
class mul_simple(width:Int) extends Module with riscv_macros{
    ...
    val input_ready_to_be = Wire(Bool())
  ...
  // Define the registers for storing the intermediate results of the multiplication
    ...
    cal_start := Mux(io.input_valid && input_ready_to_be,1.U.asBool,Mux(counter === (width - 1).U,0.U.asBool,cal_start))
    counter := Mux(cal_start ,counter + 1.U,0.U)
    signed  := Mux(io.input_valid && input_ready_to_be , io.signed  ,signed)
    val result_reg = RegInit(0.U((width * 2).W)) 

    regs(0) := Mux(io.input_valid && input_ready_to_be,Mux(io.signed,sign_extend(io.data(0),width,width * 2),io.data(0)),Mux(cal_start,regs(0) << 1,regs(0)));
    regs(1) := Mux(io.input_valid && input_ready_to_be,io.data(1),Mux(cal_start,regs(1) >> 1,regs(1)));
    ...
    input_ready_to_be := !cal_start
    io.input_ready := input_ready_to_be
}
...
class div_simple(width:Int) extends Module with riscv_macros{
  ...
  // Define the registers for storing the intermediate results of the multiplication
    ...
    val cal_start = RegInit(0.U.asBool)
    cal_start := Mux(io.input_valid && input_ready_to_be,1.U.asBool,Mux(counter === (width ).U,0.U.asBool,cal_start))
    counter := Mux(cal_start ,counter + 1.U,0.U)
    signed  := Mux(io.input_valid && input_ready_to_be , io.signed  ,signed)
...
//被除数
    ...
    regs(0) := Mux(io.input_valid && input_ready_to_be,data_in(0),Mux(cal_start,regs0_to_be,regs(0)));
    // regs0_to_be := 
    regs(1) := Mux(io.input_valid && input_ready_to_be,data_in(1),regs(1));
...
class accumulate_simple(width:Int,one_cal_times:Int) extends Module with riscv_macros{
  ...
  // Define the registers for storing the intermediate results of the multiplication
    ...
    cal_times := Mux(io.input_valid && input_ready_to_be,io.data(data_times),cal_times)
    cal_start := Mux(io.input_valid && input_ready_to_be,1.U.asBool,Mux(counter === (width ).U,0.U.asBool,cal_start))
    counter := Mux(cal_start ,counter + 1.U,0.U)
    signed  := Mux(io.input_valid && input_ready_to_be, io.signed  ,signed)
    ...
    regs(regs_under_add) := Mux(io.input_valid && input_ready_to_be,io.data(data_source),caling_data(one_cal_times - 1))
    regs(regs_for_mul_add) := Mux(io.input_valid && input_ready_to_be,io.data(data_source),regs(regs_for_mul_add) + one_cal_times.U)
...
class mul_hard(width:Int) extends Module with riscv_macros{
 ...
 // 0 -> x ; 1 -> y
    ...
    regs(0) := Mux(io.input_valid && input_ready_to_be,io.data(0),regs(0));
    x_fu  :=  Mux(io.input_valid && input_ready_to_be,(~(io.data(0)) + 1.U),x_fu);
    regs(1) := Mux(io.input_valid && input_ready_to_be,io.data(1),regs(1));

    //现在开始搭建csa
    
    val cal_module = Module(new wallace_adder(width * 2,width / 2) )
    println(cal_module.iteration_timers)
    cal_start := Mux(io.input_valid && input_ready_to_be,1.U.asBool,Mux(counter === cal_module.iteration_timers.U - 1.U,0.U.asBool,cal_start))
