...
--*/
{
   ...
   else
   {
	   ...
	   if (temp == NULL)
	   {
	    ...
	    // fseek(fpWord, 0, SEEK_END);
  		// long long_size = ftell(fpWord);
		// printf("length is %ld\n",long_size);
	   fseek(fpWord, 0, SEEK_SET);
	   int i_length = fread(temp, 1, i, fpWord);

	   printf("i length is %d  i is %d\n",i_length,i);
	   if (i_length/*fread(temp, 1, i, fpWord)*/ < (size_t)i)
	   {
		   fclose(fpWord);
		   fclose(fpMsg);
		   return -1;
