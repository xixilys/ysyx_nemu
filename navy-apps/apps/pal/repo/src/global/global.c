...
--*/
{
   ...
   // Convert the DOS-style data structure to WIN-style data structure
   //
      // printf("gpGlobals->g.rgObject[w].item.wScriptOnEquip is %d!!!!!!？？？？？!!\n",gpGlobals->g.rgObject[204].item.wScriptOnEquip);
      // printf("max_objects is %d\n",MAX_OBJECTS);
   for (i = 0; i < MAX_OBJECTS; i++)
   {
      memcpy(&gpGlobals->g.rgObject[i], &s->rgObject[i], sizeof(OBJECT_DOS));
      if(s->rgObject[i].item.wScriptOnEquip != 0) {
         // printf(" num is %d s->rgObject is %d\n",i,s->rgObject[i].item.wScriptOnEquip);
      }
...
{
   PAL_InitGlobalGameData();
   // int w = gpGlobals->g.PlayerRoles.rgwEquipment[0][0];
   // printf("gpGlobals->g.rgObject[w].item.wScriptOnEquip is %d!!!!!!!!\n",gpGlobals->g.rgObject[204].item.wScriptOnEquip);
