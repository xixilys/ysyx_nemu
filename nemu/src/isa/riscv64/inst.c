...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , U, R(dest) = s->pc + 4 ;src1 = src1>>12;s->dnpc = s->pc + immu_jal(src1);get_jal_type(s->pc,s->dnpc,jal_or_jalr,0,src2));



  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc + 4 ;s->dnpc = (src2 + src1)&0xfffffffffffffffe;get_jal_type(s->pc,s->dnpc,jal_or_jalr, BITS(s->isa.inst.val, 19, 15),src2)); 
  ...
  INSTPAT("000000? ????? ????? 001 ????? 00110 11", slliw  , I, R(dest) = SEXT((uint32_t)src1<<(src2&0x3f),32));
  INSTPAT("000000? ????? ????? 101 ????? 00110 11", srliw  , I, R(dest) = SEXT((uint32_t)src1>>(src2&0x3f),32));
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, R(dest) = SEXT((int32_t)src1>>(src2&0x3f),32));  
